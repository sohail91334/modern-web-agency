    ...
    <footer className="relative overflow-hidden bg-slate-50 border-t border-slate-100 text-slate-500">
      ...
      <div className="container-px relative">
            ...
            {Object.entries(footerLinks).map(([heading, links]) => (
              <div key={heading}>
                ...
                <ul className="space-y-3">
                  {links.map((l) => (
                    <li key={l.to}>
                      <Link href={l.to} className="text-sm transition hover:text-brand-600">
                        {l.label}
                      </Link>
                    </li>
                  ))}
        ...
        <div className="flex flex-col items-center justify-between gap-4 border-t border-slate-200 py-6 text-xs sm:flex-row">
          <p>&copy; {new Date().getFullYear()} Modern Web Agency. All rights reserved.</p>
          <div className="flex items-center gap-6">
            <Link href="/about" className="transition hover:text-brand-600">Privacy</Link>
            <Link href="/about" className="transition hover:text-brand-600">Terms</Link>
            <Link href="/contact" className="transition hover:text-brand-600">Support</Link>
          </div>
        </div>
      </div>
    </footer>
